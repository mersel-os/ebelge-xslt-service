        ...
        <xsl:choose>
            ...
            <xsl:when test="$schemas//iso:*">
                <iso:schema>
                    <xsl:attribute name="queryBinding"
                                   select="if ($defaultQueryBinding) then $defaultQueryBinding else 'xslt2'"/>

                    <!-- get header-type elements - eg title and especially ns -->
                    <!-- title (just one) -->
                    <xsl:apply-templates select="$schemas//xs:appinfo/iso:title[1]"
                                         mode="copyAndAddLocationAttributes"/>
                    ...
                    <iso:diagnostics>
                        <xsl:apply-templates select="$schemas//xs:appinfo/iso:diagnostics/*"
                                             mode="copyAndAddLocationAttributes"/>
                    </iso:diagnostics>
                </iso:schema>
            </xsl:when>
            <xsl:otherwise>
                <sch:schema>
                    ...
                    <!-- get header-type elements - eg title and especially ns -->
                    <!-- title (just one) -->
                    <xsl:apply-templates select="$schemas//xs:appinfo/sch:title[1]"
                                         mode="copyAndAddLocationAttributes"/>
                    <!-- get remaining schematron schema children -->
                    <!-- get non-blank namespace elements, dropping duplicates -->
                    ...
                    <sch:diagnostics>
                        <xsl:apply-templates select="$schemas//xs:appinfo/sch:diagnostics/*"
                                             mode="copyAndAddLocationAttributes"/>
                    </sch:diagnostics>
                </sch:schema>
            </xsl:otherwise>
