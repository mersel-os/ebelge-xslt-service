    ...
    <xsl:template match="iso:include" mode="resolveIncludes">
        <xsl:choose>
                ...
                <xsl:choose>
                    <xsl:when test="$fragment-id!=''">
                        ...
                        <xsl:variable name="doc"
                                      select="if (doc-available(resolve-uri($document-uri, base-uri(.)))) then document($document-uri,.) else ()"/>
                        <!-- End patch -->
                        <xsl:choose>
            ...
            <xsl:otherwise>
                <!-- Oxygen patch - use doc available instead of loading document -->
                <xsl:variable name="doc"
                              select="if (doc-available(resolve-uri(@href, base-uri(.)))) then document(@href,.) else ()"/>
                <!-- End patch -->
                <xsl:if test="not($doc)">
                    <xsl:message terminate="no">
                        <xsl:text>Error:Unable to open referenced included file: </xsl:text>
