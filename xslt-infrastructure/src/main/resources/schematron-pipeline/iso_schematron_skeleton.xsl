...
<xsl:stylesheet version="2.0"
                ...
                extension-element-prefixes="exsl  msxsl iso"
    ...
    <!-- SCHEMA -->
    <!-- Default uses XSLT 2 (upgraded from 1.0 for XPath 2.0 function support: xs:date(), etc.) -->
    ...
    <xsl:template name="generate-default-rules">
        ...
        <axsl:template match="*" mode="schematron-get-full-path">
            <xsl:choose>
                <xsl:when
                    test="/iso:schema[translate(@queryBinding, 'XSLT', 'xslt')='xslt2' or translate(@queryBinding, 'XSLT', 'xslt')='xslt3']">
                    <!-- XSLT2 syntax -->

                    <!-- Oxygen patch - EXM-33325 XML resources are assembled using XInclude -->
                    <axsl:variable name="sameUri">
                        <axsl:value-of select="saxon:system-id() = parent::node()/saxon:system-id()"
			...
			count(preceding-sibling::*[name()=name(current())])"/>
                            <axsl:if test="$p&gt;1">[<axsl:value-of select="$p"/>]
                            </axsl:if>
                        </axsl:when>
                        <axsl:otherwise>
    ...
    <xsl:template match="iso:rule[not(@abstract='true')] ">
        ...
        <axsl:template match="{@context}"
                       priority="{100 + count(../*) - count(preceding-sibling::*)}"
                       mode="M{count(../preceding-sibling::*)}">
            <xsl:copy-of select="@*[namespace-uri() != '']"/>
    ...
    <xsl:template match="text()" mode="text">
        <!-- EXM-30077: Because the normalize-space will remove the leading and trailing whitespaces,
        ...
        <axsl:text>
            <xsl:value-of select="substring($normalizedText, 2, string-length($normalizedText) - 2)"/>
        </axsl:text>
    </xsl:template>
    ...
    <xsl:template name="process-message">
        ...
        <xsl:if test="$diagnostics!=''">
            <axsl:text>Diagnostics: [<xsl:value-of select="$diagnostics"/>]
            </axsl:text>
            <xsl:call-template name="diagnosticsSplit">
