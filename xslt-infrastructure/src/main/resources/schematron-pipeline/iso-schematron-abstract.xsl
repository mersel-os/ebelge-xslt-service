        ...
        <xsl:copy>
            ...
            <xsl:if test="not($ap)">
                <xsl:message terminate="no">
                    Error: Cannot find abstract pattern
                    <xsl:value-of select="@is-a"/> referred from pattern <xsl:value-of select="@id"/>.
                </xsl:message>
            </xsl:if>
            <xsl:if test="$ap[2]">
                <xsl:message terminate="no">
                    Error: More than one definitions for abstract pattern
                    <xsl:value-of select="@is-a"/> referred from pattern <xsl:value-of select="@id"/>.
                </xsl:message>
            </xsl:if>
            ...
            <xsl:for-each select="iso:param">
                ...
                <xsl:if test="@name = $ap[1]//iso:let/@name/substring(., 1, $paramLength)">
                    <xsl:message terminate="no">
                        ...
                        <xsl:value-of
                            select="$ap[1]//iso:let[starts-with(@name, current()/@name)]/concat($apos, @name, $apos)"
                            separator=", "/>.
            ...
            <xsl:variable name="oxyParams" select="$ap/oxyP:parameters/oxyP:parameter"
                          xmlns:oxyP="http://oxygenxml.com/ns/schematron/params"/>
            ...
            <xsl:if test="$oxyParams">
                ...
                <xsl:for-each select="$oxyParams">
                    <xsl:if test="not(current()/oxyP:name = $params/@name)"
                            xmlns:oxyP="http://oxygenxml.com/ns/schematron/params">
                        <xsl:apply-templates select="$pattern" mode="errorMsg">
                            <xsl:with-param name="message">
                                Error: The "<xsl:value-of select="current()/oxyP:name"/>" parameter is not passed to the
                                abstract pattern.
                            </xsl:with-param>
                        </xsl:apply-templates>
                    </xsl:if>
                ...
                <xsl:for-each select="$params">
                    <xsl:if test="not(current()/@name = $oxyParams/oxyP:name)"
                            xmlns:oxyP="http://oxygenxml.com/ns/schematron/params">
                        <xsl:apply-templates select="current()" mode="errorMsg">
                            <xsl:with-param name="message">
                                Error: The "<xsl:value-of select="current()/@name"/>" parameter is not defined in the
                                abstract pattern.
                            </xsl:with-param>
                        </xsl:apply-templates>
                    </xsl:if>
            ...
            <xsl:otherwise>
                ...
                <xsl:if test="matches($after,'^[a-zA-Z0-9_\-]')">
                    <xsl:apply-templates select="$param" mode="errorMsg">
                        <xsl:with-param name="message">
                            Warning: The "<xsl:value-of select="$param/@name"/>" parameter is not used correctly in the
                            following abstract pattern text "<xsl:value-of select="$value"/>". A separator is expected
