...
export interface SchematronParam {
  key: string;
  value: string;
}
  ...
  return useMutation({
    mutationFn: async (params: ValidateParams) => {
      ...
      if (params.parameters && params.parameters.length > 0) {
        const validParams = params.parameters.filter(
          (p) => p.key.trim() !== ""
        );
...
// ─── Admin: Auto-Generated Content ──────────────────────────────────

export function useAutoGeneratedContent(path: string) {
  return useQuery({
    queryKey: ["auto-generated-content", path],
    queryFn: () =>
      customInstance<AutoGeneratedContentResponse>({
        url: "/v1/admin/auto-generated/content",
        method: "GET",
        params: { path },
      }),
    enabled: !!path,
